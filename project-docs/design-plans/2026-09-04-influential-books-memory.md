...
**Dependencies:** `HARDCOVER_TOKEN` in `book-tools/.env` (gitignored). Field names confirmed by a live query on 2026-09-04 (library: 2,544 `user_books` rows per `user_books_aggregate`; `me.books_count` is a cached counter that read 1,254 and is not the row count; `goals.progress` is likewise a stored number that bulk-imported reads do not bump — it read 10 against 31 `user_book_reads` finished in 2026 on 2026-09-05, and re-saving the goal in the app recomputed it):
...
**Prior art (scanned 2026-09-05).** No maintained FOSS Hardcover client exists to build on: [hardcover-mcp](https://github.com/alexsowt/hardcover-mcp) (Python, read-only, unlicensed, 1 star), [book-effect](https://github.com/kevinmichaelchen/book-effect) (TypeScript, 1 star), and nothing on PyPI or npm. [BookLore](https://github.com/booklore-app/booklore) (AGPL-3.0, active) syncs Hardcover metadata and is worth reading for rate-limit handling, not depending on. No tool was found that mines a personal corpus for book mentions. The stdlib client stays. Status id 4 is "Paused" per a community post citing the API docs (https://www.emgoto.com/hardcover-book-api/); docs.hardcover.app itself refused automated fetches during the scan.
